...
<body class="bg-gray-950 text-white font-sans min-h-screen flex flex-col antialiased" style="font-feature-settings: 'kern' 1; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;">
    <!-- Header -->
    <header class="bg-gray-950 shadow-md sticky top-0 z-50">
        <div class="container mx-auto flex justify-between items-center px-6 py-4">
            <a href="{{ auth()->check() ? route('workspace') : '/' }}" class="text-2xl font-bold text-white hover:text-blue-500 transition">
                Tasko
            </a>
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('login') }}" class="text-white hover:text-blue-500 transition">Connexion</a>
                <a href="{{ route('register') }}" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">Inscription</a>
            </div>
            @endauth
        </div>
    </header>
    ...
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
        ...
        x-transition:leave-start="opacity-100 transform translate-x-0"
        x-transition:leave-end="opacity-0 transform translate-x-full"
        class="fixed top-20 right-4 z-[9999] bg-green-600 text-white px-6 py-3 rounded-lg shadow-2xl flex items-center gap-3 min-w-[300px]">
    ...
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
        ...
        x-transition:leave-start="opacity-100 transform translate-x-0"
        x-transition:leave-end="opacity-0 transform translate-x-full"
        class="fixed top-20 right-4 z-[9999] bg-red-600 text-white px-6 py-3 rounded-lg shadow-2xl flex items-center gap-3 min-w-[300px]">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
        </svg>
    ...
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
        ...
        x-transition:leave-start="opacity-100 transform translate-x-0"
        x-transition:leave-end="opacity-0 transform translate-x-full"
        class="fixed top-20 right-4 z-[9999] bg-yellow-600 text-white px-6 py-3 rounded-lg shadow-2xl flex items-center gap-3 min-w-[300px]">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
    ...
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
        ...
        x-transition:leave-start="opacity-100 transform translate-x-0"
        x-transition:leave-end="opacity-0 transform translate-x-full"
        class="fixed top-20 right-4 z-[9999] bg-blue-600 text-white px-6 py-3 rounded-lg shadow-2xl flex items-center gap-3 min-w-[300px]">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
